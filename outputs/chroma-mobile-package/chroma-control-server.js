...
const PAGE = "chroma-cross-screen.html";
...
async function handler(req, res) {
  ...
  if (url.pathname === "/" || url.pathname === "/display" || url.pathname === "/control") {
    const suffix = url.pathname === "/display" ? "?mode=display" : url.pathname === "/control" ? "?mode=control" : "";
    res.writeHead(302, { location: `/${PAGE}${suffix}` });
    return res.end();
  }
  ...
  if (url.pathname === "/api/register" && req.method === "POST") {
    ...
    devices.set(id, {
      id,
      name: String(body.name || previous?.name || `设备 ${id.slice(-4)}`).slice(0, 40),
    ...
    return send(res, 200, JSON.stringify({ ok: true, state: nextState }), "application/json; charset=utf-8");
  }
  ...
  if (url.pathname === "/qr.svg") {
    const text = url.searchParams.get("text") || `http://${getLanIp()}:${PORT}/${PAGE}`;
    try {
  ...
  if (!filePath.startsWith(ROOT) || !fs.existsSync(filePath)) return send(res, 404, "Not found");
...
http.createServer((req, res) => {
  ...
  const url = `http://${getLanIp()}:${PORT}/${PAGE}`;
